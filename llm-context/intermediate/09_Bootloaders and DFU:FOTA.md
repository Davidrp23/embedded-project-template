...
> **Note**: It's also possible to have second-stage *upgradable* bootloaders, but that's beyond this course scope. See [Bootloaders and Device Firmware Updates](https://docs.nordicsemi.com/bundle/ncs-latest/page/nrf/app_dev/bootloaders_dfu/index.html).
...
Official documentation: [Bootloaders and Device Firmware Updates](https://docs.nordicsemi.com/bundle/ncs-latest/page/nrf/app_dev/bootloaders_dfu/index.html).

> **Important v3.0 Change**: Sysbuild is now **mandatory** for multi-image builds, including any project that uses MCUboot. This replaces the legacy child image approach.
...
*   **Documentation:** [Using MCUboot in nRF Connect SDK](https://docs.nordicsemi.com/bundle/ncs-latest/page/mcuboot/index.html).
...
*   **Image Compression (v3.0+):** MCUboot now supports **image compression** using LZMA2 algorithm. This can significantly reduce the size of update images, saving bandwidth for FOTA updates. Enable with `CONFIG_MCUBOOT_COMPRESSED_IMAGE_SUPPORT=y` in the MCUboot configuration.
...
*   **Sysbuild:** **Required** for building MCUboot alongside the application in NCS v3.0+ (as covered in Lesson 8). Sysbuild manages the build of all images in a multi-image application.
...
*   [NCS Bootloaders and DFU Overview](https://docs.nordicsemi.com/bundle/ncs-latest/page/nrf/app_dev/bootloaders_dfu/index.html)
*   [MCUboot Documentation (Upstream/Zephyr)](https://docs.nordicsemi.com/bundle/ncs-latest/page/mcuboot/design.html)
*   [Using MCUboot in NCS](https://docs.nordicsemi.com/bundle/ncs-latest/page/mcuboot/index.html)
*   Device Specific DFU Guides (e.g., [nRF52 FOTA](https://docs.nordicsemi.com/bundle/ncs-latest/page/nrf/device_guides/nrf52/fota_update.html))
*   [Zephyr SMP Server Sample](https://docs.nordicsemi.com/bundle/ncs-latest/page/zephyr/samples/subsys/mgmt/mcumgr/smp_svr/README.html)
*   NCS Samples often include MCUboot/DFU options (e.g., Peripheral LBS, Matter Door Lock).
*   [NCS MCUboot Tests](https://docs.nordicsemi.com/bundle/ncs-latest/page/nrf/tests/bootloader/mcuboot/README.html)
*   [NCS FOTA Download Library Docs](https://docs.nordicsemi.com/bundle/ncs-latest/page/nrf/libraries/others/fota_download.html)
*   Cloud FOTA Guides (e.g., [nRF Cloud FOTA](https://docs.nordicsemi.com/bundle/ncs-latest/page/nrf/protocols/nrf_cloud/nrf_cloud_fota.html), [AWS FOTA](https://docs.nordicsemi.com/bundle/ncs-latest/page/nrf/protocols/aws/fota.html), [Azure FOTA](https://docs.nordicsemi.com/bundle/ncs-latest/page/nrf/protocols/azure_iot_hub/fota.html))

### DFU transmission tools (SMP Clients)

*   **PC:**
    *   [MCUmgr CLI](https://docs.nordicsemi.com/bundle/ncs-latest/page/zephyr/services/device_mgmt/mcumgr.html) (Command line tool)
...
### Flashing with nrfutil (v3.0+)

> **Important v3.0 Change**: `west flash` now uses **nrfutil** instead of `nrfjprog` for flashing Nordic devices. The `nrfutil` tool provides a unified interface and supports all Nordic devices including the latest series.

To flash your application:
```bash
west flash
```

If you need to use `nrfjprog` directly for specific tasks, you can still install it separately, but `nrfutil` is the recommended tool for NCS v3.0+.
...
Configuration details: [Simultaneous multi-image DFU](https://docs.nordicsemi.com/bundle/ncs-latest/page/nrf/app_dev/bootloaders_dfu/mcuboot_multi_image.html). Exercises will cover both methods.
...
*   **(Recommended) Disable DK Mass Storage:** Prevents interference with UART. Use nrfutil:
    ```bash
    # List connected devices to find your DK's serial number
    nrfutil device list
    # Disable mass storage (replace SERIAL_NUMBER with your DK's serial number)
    nrfutil device x-msd --serial-number <SERIAL_NUMBER> --msd-state off
    ```
    Power-cycle the DK afterward. Re-enable later with `--msd-state on` if needed.

### Exercise steps

Open the exercise base code: `l9/l9_e1` (Blinky sample) using "Copy a sample".

> **Note on Board Targets**: In NCS v3.0+, board targets use `/` instead of `_`. For example:
> - nRF52840 DK: `nrf52840dk/nrf52840`
> - nRF5340 DK Application Core: `nrf5340dk/nrf5340/cpuapp`
> - nRF5340 DK Network Core: `nrf5340dk/nrf5340/cpunet`
...
*   To update the **network core** using DFU from the application (non-simultaneous):
    *   Build the network core image separately (targeting `nrf5340dk/nrf5340/cpunet`).
...
Reference: [nRF91 FOTA Documentation](https://docs.nordicsemi.com/bundle/ncs-latest/page/nrf/app_dev/bootloaders_dfu/nrf91_fota.html).
...
> **Note on Board Targets**: Use the new board target format:
> - nRF9160 DK: `nrf9160dk/nrf9160/ns`
> - nRF9161 DK: `nrf9161dk/nrf9161/ns`

1.  **Build and Flash Base:** Build for your nRF91 DK (e.g., `nrf9160dk/nrf9160/ns` target). Flash. Connect serial terminal. Verify connection to nRF Cloud logs.
...
5.  **Setup FOTA in nRF Cloud:** (See [nRF Cloud FOTA Docs](https://docs.nordicsemi.com/bundle/ncs-latest/page/nrf/protocols/nrf_cloud/nrf_cloud_fota.html))
...
Reference: [AWS FOTA Documentation](https://docs.nordicsemi.com/bundle/ncs-latest/page/nrf/protocols/aws/fota.html).
...
2.  **Set up AWS IoT Core:** (Requires AWS account)
    ...
    *   Follow steps in [AWS IoT Setup Guide](https://docs.nordicsemi.com/bundle/ncs-latest/page/nrf/protocols/aws/getting_started/index.html) or the brief steps below:
...
5.  **Perform FOTA with AWS IoT Core:**
    ...
    5.5 **Create FOTA Job:** Follow steps in [AWS FOTA: Creating a FOTA job](https://docs.nordicsemi.com/bundle/ncs-latest/page/nrf/protocols/aws/fota.html#creating-a-fota-job). This involves:
...
## Summary of v3.0 Changes for Bootloaders/DFU

This section summarizes the key changes in nRF Connect SDK v3.0+ that affect bootloader and DFU workflows:

| Feature | v2.x | v3.0+ |
|---------|------|-------|
| Multi-image builds | Child image or Sysbuild | **Sysbuild mandatory** |
| Board targets | `nrf52840dk_nrf52840` | `nrf52840dk/nrf52840` |
| Flashing tool | nrfjprog | **nrfutil** |
| MCUboot compression | Not available | **LZMA2 supported** |
| Documentation URLs | developer.nordicsemi.com | **docs.nordicsemi.com** |

### Migration Checklist

When migrating DFU/FOTA projects from v2.x to v3.0:

1. **Update board targets** - Change underscores to forward slashes
2. **Ensure Sysbuild is used** - Add `sysbuild.conf` with MCUboot configuration
3. **Update documentation links** - Use docs.nordicsemi.com URLs
4. **Consider image compression** - Enable `CONFIG_MCUBOOT_COMPRESSED_IMAGE_SUPPORT=y` for bandwidth savings
5. **Install nrfutil** - Ensure nrfutil is installed for flashing operations

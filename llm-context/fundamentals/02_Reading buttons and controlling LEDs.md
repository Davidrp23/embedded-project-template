...
*(Instructions corresponding to nRF Connect SDK v3.0.0 - v3.2.0)*
...
These hardware details are all described in the devicetree file for the nRF52833 DK. Let's examine this file, available in `<install_path>/zephyr/boards/nordic/nrf52833dk/nrf52833dk_nrf52833.dts`

> **Note**
> Starting with nRF Connect SDK v2.7.0, the hardware model was updated (Hardware Model v2). Board files are now organized under `boards/<vendor>/<board_name>/` (e.g., `boards/nordic/nrf52833dk/`) instead of the previous `boards/arm/<board_name>/` structure.

The devicetree file for the DK includes the devicetree for the specific SoC variant used in the development kit. In the case of the nRF52833 DK, it is the file `nrf52833_qiaa.dtsi` available in the directory `<install_path>/zephyr/dts/arm/nordic`. This file is used because it corresponds to the package variant and function variant of the SoC used on the nRF52833 DK. The `I` in `DTSI` stands for Include. `dtsi` files generally contain SoC-level definitions. It also includes the pin mapping, defined in `nrf52833dk_nrf52833-pinctrl.dtsi`.
...
The devicetree file relies on the pin control devicetree file for the pin mappings of the various nodes (except for LEDs and buttons). As seen in the image description above, the `&uart0` node has references to the nodes `&uart0_default` and `&uart0_sleep`, which are defined in this file. The pin control devicetree file is found in `<install_path>/zephyr/boards/nordic/nrf52833dk/nrf52833dk_nrf52833-pinctrl.dtsi`.

*   The `&pinctrl` node (defined in `<install_path>/zephyr/dts/bindings/pinctrl/nordic,nrf-pinctrl.yaml`) includes all device pin configurations in its sub-nodes.
...
Now, examine the SoC variant devicetree `nrf52833_qiaa.dtsi` available in the directory `<install_path>/zephyr/dts/arm/nordic`.
...
The SoC devicetree contains SoC-level hardware descriptions for all the peripherals and system blocks. Examine the base SoC devicetree `nrf52833.dtsi` available in the directory `<install_path>/zephyr/dts/arm/nordic`.
...
> **Note**
> The function `device_get_binding()` has been removed from Zephyr and should not be used. Always use `DEVICE_DT_GET()` to retrieve the device pointer.
> Using `DEVICE_DT_GET()` has the benefit of failing at build time if the device was not allocated by the driver, for instance, if it does not exist in the devicetree or has the status `disabled`.
> In addition, unlike the deprecated `device_get_binding()`, it does not perform a run-time string comparison, which improves performance.
...
```
All GPIO flags are documented [here](https://docs.nordicsemi.com/bundle/ncs-latest/page/zephyr/hardware/peripherals/gpio.html#gpio-flags).
...
2.  **Interrupt method**
    ...
    1.  **Configure the interrupt on a pin.**
        ...
        ```
        All interrupt flag options are documented [here](https://docs.nordicsemi.com/bundle/ncs-latest/page/zephyr/hardware/peripherals/gpio.html#interrupt-configuration), under GPIO interrupt configuration flags.
...
The full API documentation for GPIO generic interface is available [here](https://docs.nordicsemi.com/bundle/ncs-latest/page/zephyr/hardware/peripherals/gpio.html).
...
1.  **Clone the nRF Connect SDK Fundamentals GitHub repository.** Use either the VS Code GUI (1.1) or the command line (1.2).
    ...
    > The `main` branch corresponds to nRF Connect SDK v3.2.0. For other SDK versions, select the appropriate branch (e.g., `v2.9.0-v2.7.0` for older versions).

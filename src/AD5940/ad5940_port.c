...
{
    // Zephyr permite pasar NULL si no quieres enviar o recibir, 
    // pero la librería de AD5940 siempre gestiona ambos buffers.
    ...
    // Bajamos la frecuencia a 500kHz para asegurar estabilidad total
    static const struct spi_config spi_cfg_stable = {
        .frequency  = 500000, 
        .operation  = SPI_WORD_SET(8) | SPI_TRANSFER_MSB,
    };

    int ret = spi_transceive(spi_dev, &spi_cfg_stable, &tx_set, &rx_set);
    if (ret < 0) {
        printf("SPI Error grave: %d\n", ret);
    }
}
...
void AD5940_CsClr(void)
{
	...
	k_busy_wait(250);  /* Post-assertion delay: AD5940 needs 14-40us to wake from hibernate */
}
...
uint32_t AD5940_MCUResourceInit(void *pCfg)
{
    ...
    /* (Opcional) Si quieres evitar un warning de variable no usada: */
    (void)pCfg; 
